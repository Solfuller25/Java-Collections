...
public class OrgChart {
    ...
    public boolean addEmployee(Employee employee) {
    	if (this.orgChart.contains(employee) || employee == null) {
    		return true;
    	} else if (employee.hasManager()) {
    		// Employee has manager and manager is NOT part of orgChart
    		if (!this.orgChart.contains(employee.getManager())) {
    			// This case is recursive
    			// Get list of managers, and add them to the set
    			List<Manager> managerList = employee.getChainOfCommand();
    			for (Manager m : managerList) {
    				if (!this.orgChart.contains(m)) {
    					addEmployee(m);
    				}
    			}
    			// Now add employee to the set
    			return this.orgChart.add(employee);
    		} else {
        		// Employee has manager and manager IS part of orgChart
    			return this.orgChart.add(employee);
    		}
    ...
    public Set<Employee> getDirectSubordinates(Manager manager) {
    	...
    	// Check for manager to be in org chart. If not, return empty set
    	if (!orgChart.contains(manager) || orgChart.isEmpty()) {
    		return subordinates;
    	} else {
    		// Loop through org chart checking if employee is direct subordinate of manager
        	for (Employee emp : orgChart) {
        		if (emp.hasManager()) {
